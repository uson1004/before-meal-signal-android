...
프로필 화면의 식전 알림은 현재 로컬 프로토타입 설정입니다. 사용자는 아침·점심·저녁 알림 대상과 10·20·30분 전 리드타임을 고를 수 있고, 앱은 오늘 급식 데이터가 있는 식사만 요약에 반영합니다.

실제 OS 알림 예약은 아직 포함하지 않았습니다. `AlarmManager`/`WorkManager`, 알림 채널, 런타임 권한, 저장소 연동은 별도 구현 범위입니다.
